function gradStructure = plot_gradient_trends( figNo, Title_to_Plot)
%
...
yearMonthIndices = (2024-2001) * 12;

gradStructure.day_counts = nan((2024-2001)*12, 180, 360);
gradStructure.night_counts = nan((2024-2001)*12, 180, 360);
...
gradStructure.slope = nan(180, 360);
gradStructure.intercept = nan(180, 360);


xDateTimeIndex = 1:yearMonthIndices;
...
for iFile=1:length(filelist)
    ...
    dayCounts = ncread(filename, 'day_pixel_count');
    nightCounts = ncread(filename, 'night_pixel_count');

    gradStructure.day_counts(dateTimeIndex,:,:) = dayCounts;
    gradStructure.night_counts(dateTimeIndex,:,:) = nightCounts;

    gradStructure.dayMeanEastGrad(dateTimeIndex,:,:) = ncread(filename, 'day_sum_eastward_gradient') ./ dayCounts;
    gradStructure.dayMeanNorthGrad(dateTimeIndex,:,:) = ncread(filename, 'day_sum_northward_gradient') ./ dayCounts;
    gradStructure.dayMeanGradMag(dateTimeIndex,:,:) = ncread(filename, 'day_sum_magnitude_gradient') ./ dayCounts;

    gradStructure.nightMeanEastGrad(dateTimeIndex,:,:) = ncread(filename, 'night_sum_eastward_gradient') ./ nightCounts;
    gradStructure.nightMeanNorthGrad(dateTimeIndex,:,:) = ncread(filename, 'night_sum_northward_gradient') ./ nightCounts;
    gradStructure.nightMeanGradMag(dateTimeIndex,:,:) = ncread(filename, 'night_sum_magnitude_gradient') ./ nightCounts;

    gradStructure.meanEastGrad(dateTimeIndex,:,:) = (ncread(filename, 'day_sum_eastward_gradient') + ncread(filename, 'night_sum_eastward_gradient')) ./ (dayCounts + nightCounts);
    gradStructure.meanNorthGrad(dateTimeIndex,:,:) = (ncread(filename, 'day_sum_northward_gradient') + ncread(filename, 'night_sum_northward_gradient')) ./ (dayCounts + nightCounts);
    gradStructure.meanGradMag(dateTimeIndex,:,:) = (ncread(filename, 'day_sum_magnitude_gradient') + ncread(filename, 'night_sum_magnitude_gradient')) ./ (dayCounts + nightCounts);
end

%% Get trends

dayNightAll = {'Day' 'Night' 'All'};

tempDayArray = gradStructure.dayMeanGradMag;
tempNightArray = gradStructure.nightMeanGradMag;
tempAllArray = gradStructure.meanGradMag;
for iLon=1:360
    for iLat=1:180
        for idna=1:3
            eval(['y = squeeze(temp' dayNightAll{idna} 'Array(:,iLat,iLon));'])
            nn = find(isnan(y)==0);
            if isempty(nn) == 0
                if length(nn) > 2
                    yGood = y(nn);
                    ...
                    eval(['gradStructure.slope_' dayNightAll{idna} '(iLat,iLon) = pp(1);'])
                    eval(['gradStructure.intercept_' dayNightAll{idna} '(iLat,iLon) = pp(2);'])
                end
            end
        end
    end
...
%% Plot slope of gradient magnitude day and night together.

figure(figNo)
clf

if plot_with_imagesc
    imagesc(lonVector, latVector, gradStructure.slope_All)
    colormap("jet")
else
    % Set land areas to NaN
    maskedData = gradStructure.slope_All;
...
title( 'Combined Day/Night Gradient Magnitude Trends', fontsize=titleFontSize)

% Plot slope of gradient magnitude day.

figure(figNo+1)
clf

if plot_with_imagesc
    imagesc(lonVector, latVector, gradStructure.slope_Day)
    colormap("jet")
else
    % Set land areas to NaN
    maskedData = gradStructure.slope_Day;
    maskedData(inLand) = NaN;
...
ylabel('Latitude')
set(gca, fontsize=axisFontSize, ydir="normal")
title( 'Daytime Gradient Magnitude Trends', fontsize=titleFontSize)

% Plot slope of gradient magnitude night.

figure(figNo+2)
clf

if plot_with_imagesc
    imagesc(lonVector, latVector, gradStructure.slope_Night)
    colormap("jet")
else
    % Set land areas to NaN
    maskedData = gradStructure.slope_Night;
    maskedData(inLand) = NaN;
...
ylabel('Latitude')
set(gca, fontsize=axisFontSize, ydir="normal")
title( 'Nighttime Gradient Magnitude Trends', fontsize=titleFontSize)
