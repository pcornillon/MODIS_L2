...
number_of_scans(iOrbit) = length(orbit_scan_line_times,2) - start_line_index + 1;
...
while iOrbit <= number_of_orbits
    ...
    if isempty(start_line_index)
        ...
        number_of_scans(iOrbit) = length(orbit_scan_line_timesT) - start_line_index + 1;

        NASA_orbit_number(iOrbit) = ncreadatt( fi_metadata,'/','orbit_number');
    end
    
        % Add 5 minutes to the previous value of time to get the time of 
        % the next granule and continue searching.
