function [file_list, granule_start_time_guess] = big_step_search(baseName, granule_start_time_guess, Trailer)
% big_step_search - searches times in filenames containing HHMMSS for one day returns the time when one is found - PCC
%
% INPUT
...
while true
    ...
    if granule_start_time_guess > Matlab_end_time
        Step = -floor( (Step / 2) * 24 * 60);
    granule_start_time_guess = granule_start_time_guess + Step;
