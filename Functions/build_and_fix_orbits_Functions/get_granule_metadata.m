...
if isempty(file_list)
    ...
    status = populate_problem_list( 10, oinfo(iOrbit).name);

elseif length(file_list) > 2
    fprintf('*** Too many metadata files for %s. Going to the next granule.\n', datestr(granule_start_time_guess, formatOut.yyyymmddThhmmss))

    status = populate_problem_list( 11, file_list(1));
    ...
    if abs(mSec(10)-mSec(1)) > 0
        ...
        status = populate_problem_list( 6, oinfo(iOrbit).ginfo(iGranule).metadata_name);
        return
    end
