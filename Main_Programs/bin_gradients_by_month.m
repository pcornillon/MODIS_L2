        ...
        if exist(outputFile) == 2
            ...
            if isempty(orbit_files) == 0
                ...
                for fileIdx = 1:length(orbit_files)
                    ...
                    [status, lat] = read_nc_variable( orbit_filename, 'latitude', month, year, data_dir, iBadOrbit, badOrbits, 'Could not read this file.');
                    if status == 0
                        [status, lon] = read_nc_variable( orbit_filename, 'longitude', month, year, data_dir, iBadOrbit, badOrbits, 'longitude does not exist.');
                        if status == 0
                            [status, east_grad] = read_nc_variable( orbit_filename, 'eastward_gradient', month, year, data_dir, iBadOrbit, badOrbits, 'eastward_gradient does not exist.');
                            if status == 0
                                [status, north_grad] = read_nc_variable( orbit_filename, 'northward_gradient', month, year, data_dir, iBadOrbit, badOrbits, 'northward_gradient does not exist.');
                                if status == 0
                                    [status, dateTime] = read_nc_variable( orbit_filename, 'DateTime', month, year, data_dir, iBadOrbit, badOrbits, 'DateTime does not exist.');
                                    if status == 0
                                        [status, time_from_start_orbit] = read_nc_variable( orbit_filename, 'time_from_start_orbit', month, year, data_dir, iBadOrbit, badOrbits, 'time_from_start_orbit does not exist.');
                                        if status == 0
