...
MATLAB_FILENAME="matlab_$(date +'%Y-%m-%d_%H-%M-%S').txt"
...
echo "Checked for the output directory, created if it did not exist." | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"

# Some output.

echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
date  | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
echo "Starting the script" | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
echo "I am $(whoami) and proud of it" | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
...
echo "I am about to fire up Matlab." 2>&1 | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
...
echo "I just started Matlab. Am still $(whoami). It should be running in the background. This script is finished." | tee -a "${LOCAL_OUTPUT_DIRECTORY}//$AWS_FIL$LOCAL_SESSION_FILENAME"
