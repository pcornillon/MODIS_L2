...
if [ "$(whoami)" != "ubuntu" ] && [ "$(whoami)" != "petercornillon" ]; then
    ...
    su ubuntu -c "/usr/local/bin/aws --profile iam_pcornillon ec2 associate-address --allocation-id eipalloc-0804440433bc44fcb --instance-id ${MYID}"

    while true; do
        myip=$(curl http://169.254.169.254/latest/meta-data/public-ipv4)
        if [[ "$myip" == "52.34.203.74" ]]; then
            break
        else
            sleep 2
        fi
...
echo "Checked for the output directory, created if it did not exist." | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"

# Some output.

echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
date  | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
echo "" | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
echo "Starting the script..." | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
echo "I am $(whoami) and proud of it" | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"

# Change to the git repo directory for this project and pull the latest changes as user ubuntu

if [ "$(whoami)" != "ubuntu" ] && [ "$(whoami)" != "petercornillon" ]; then
    echo "Pulling to $LOCAL_MATLAB_PROJECT_DIRECTORY as user ubuntu" | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
...
echo "I am about to fire up Matlab." 2>&1 | tee -a "${LOCAL_OUTPUT_DIRECTORY}/local_session_log_52_34_203_74.txt"
...
sudo -u ubuntu bash -c '
  ...
  echo "Am running in sudo submitted version of script." | tee -a "${REMOTE_OUTPUT_DIRECTORY}/remote_session_log_52_34_203_74.txt"
  cd "$REMOTE_MATLAB_PROJECT_DIRECTORY"
  echo "Pulling to $REMOTE_MATLAB_PROJECT_DIRECTORY as user $(whoami)" | tee -a "${REMOTE_OUTPUT_DIRECTORY}/remote_session_log_52_34_203_74.txt"
  git pull
  ...
  nohup matlab -batch "prj=openProject('\''$REMOTE_MATLAB_PROJECT_DIRECTORY/MODIS_L2.prj'\''); AWS_batch_07_from_01_2006_to_12_2016;" > "$REMOTE_OUTPUT_DIRECTORY/$FILENAME" 2>&1 &
